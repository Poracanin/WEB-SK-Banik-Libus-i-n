Create Codex pet row `running-right` for `bily-retrivr`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A compact friendly cream-white golden retriever puppy, soft floppy ears, warm brown eyes, happy gentle expression, readable full-body silhouette, plush-sticker toy feel, no collar, no props.. Preserve silhouette, face, palette, material, proportions, markings, and props.
